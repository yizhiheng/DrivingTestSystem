	...
	$(document).ready(
			function() {
				...
				$("input.explainButton").click(
						function() {
							var id = $(this).parent().find("input.questionId")
									.attr("id");
							$("#explain" + id).css("display", "block");
						});

				//删除错题本按钮被单击时的响应事件
				$("input.deleteError").click(
						function() {
		                var questionid = $(this).parent().find("input.questionId").attr("id");
		                var rightAnswer = $(this).parent().find("input.rightAnswer").attr("id");
		                var userid = $("#userid").attr("value");
						if (confirm("确认将本题从错题本中删除吗？")){
							deleteError(userid, questionid);
						}
						});
				...
				$(":radio").click(
						function() {
							var id = $(this).parent().find("input.questionId")
									.attr("id"); //id变量记录了此题目的id
							var rightAnswer = $(this).parent().find(
									"input.rightAnswer").attr("id"); //rightAnswer记录了正确的答案,例如optionA
							if ($(this).attr("id") == rightAnswer) {
								$(this).parent().find("span.rightAnswer").css(
										"display", "block");
								$(this).parent().find("span.wrongAnswer").css(
										"display", "none");
							} else {
								$(this).parent().find("span.wrongAnswer").css(
										"display", "block");
			...
			// ajax初始化
			function deleteError(userid, questionid) {	
			    var xmlhttp;
			    ...
			    xmlhttp.onreadystatechange = function() {
			        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			            $("#question" + questionid).find("input.deleteError").attr(
			                "disabled", "disabled"); //把此题目的加入错题本按钮设置为disabled
			            alert("删除错题成功！");
			        }
			    }
			    xmlhttp.open("GET", "http://localhost:8080/DrivingTestSystem/servlet/DeleteError?userid=" + userid + "&questionid=" + questionid, true);
			    xmlhttp.send();
			}
</script>
...
<!-- Add custom CSS here -->
<link href="css/errorsPractice.css" rel="stylesheet">
</head>
		...
		<div class="collapse navbar-collapse">
			<ul class="nav navbar-nav">
				<li class="dropdown" id="category"><a href="#"
					class="dropdown-toggle" data-toggle="dropdown"> 章节练习 <b
						class="caret"></b> </a>
				...
				<li class="active"><a href="errorsPractice.jsp">错题练习</a></li>
			...
			<div class="col-md-9">
				...
				<div class="well">
					...
					<%
						...
						String[] splitErrors = errors.split(",");
						List<String> tempList = Arrays.asList(splitErrors);
						...
						while (rs.next()) {
							...
							if (tempList.contains(questionId + "")) {
								...
								//题目的DIV
								out.println("<div id='question" + questionId
										+ "' class='row question'>"); //Div id为question+id号
								...
								if (rs.getString("pic") != null) {
									out.println("<img class='question-img img-responsive pull-right' src='queimg/"
											+ rs.getString("pic") + "'>");
								}

								//四个选项
								if (rs.getString("optionA") != null) {
									out.println("<input type='radio' name='" + questionId
											+ "' id='optionA'/>");
									out.println("<span class='options'>A."
											+ rs.getString("optionA") + "</span>");
									out.println("<br>");
								}
								if (rs.getString("optionB") != null) {
									out.println("<input type='radio' name='" + questionId
											+ "' id='optionB'/>");
									out.println("<span class='options'>B."
											+ rs.getString("optionB") + "</span>");
									out.println("<br>");
								}
								if (rs.getString("optionC") != null) {
									out.println("<input type='radio' name='" + questionId
											+ "' id='optionC'/>");
									out.println("<span class='options'>C."
											+ rs.getString("optionC") + "</span>");
									out.println("<br>");
								}
								if (rs.getString("optionD") != null) {
									out.println("<input type='radio' name='" + questionId
											+ "' id='optionD'/>");
									out.println("<span class='options'>D."
											+ rs.getString("optionD") + "</span>");
									out.println("<br>");
								}
								...
								out.println("<input type='button' value='本题解释' class='explainButton btn btn-info' id='"
										+ questionId + "'/>");

								out.println("<input type='button' value='从错题本中删除' class='deleteError btn btn-default' />");
